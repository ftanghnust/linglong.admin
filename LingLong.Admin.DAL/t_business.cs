...
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2018/9/5 22:59:47   N/A    初版
*
...
namespace LingLong.Admin.DAL
{
	...
	/// 数据访问类:t_business
	/// </summary>
	public partial class t_business
	{
        private string databaseprefix; //数据库表名前缀
        public t_business(string _databaseprefix)
		...
		public int GetMaxId()
		{
		return DbHelperMySql.GetMaxID("ID", "t_business"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from t_business");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(LingLong.Admin.Model.t_business model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into t_business(");
			strSql.Append("TrueName,Nickname,Gender,AvatarUrl,OpenId,UnionId,PhoneNumber,AppId,City,Province,Country,NativePlace,Height,Birthday,RegisterTime,IsDeleted,DeleterUserId,LastModificationTime,LastModifierUserId,CreationTime,CreatorUserId)");
			...
			parameters[3].Value = model.AvatarUrl;
			...
			parameters[14].Value = model.RegisterTime;
			parameters[15].Value = model.IsDeleted;
			parameters[16].Value = model.DeleterUserId;
			parameters[17].Value = model.LastModificationTime;
			parameters[18].Value = model.LastModifierUserId;
			parameters[19].Value = model.CreationTime;
			parameters[20].Value = model.CreatorUserId;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(LingLong.Admin.Model.t_business model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update t_business set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from t_business ");
			strSql.Append(" where ID=@ID");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.Int32)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public LingLong.Admin.Model.t_business GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,TrueName,Nickname,Gender,AvatarUrl,OpenId,UnionId,PhoneNumber,AppId,City,Province,Country,NativePlace,Height,Birthday,RegisterTime,IsDeleted,DeleterUserId,LastModificationTime,LastModifierUserId,CreationTime,CreatorUserId from t_business ");
			strSql.Append(" where ID=@ID");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.Int32)
			};
			...
			LingLong.Admin.Model.t_business model=new LingLong.Admin.Model.t_business();
		...
		/// 得到一个对象实体
		/// </summary>
		public LingLong.Admin.Model.t_business DataRowToModel(DataRow row)
		{
			LingLong.Admin.Model.t_business model=new LingLong.Admin.Model.t_business();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,TrueName,Nickname,Gender,AvatarUrl,OpenId,UnionId,PhoneNumber,AppId,City,Province,Country,NativePlace,Height,Birthday,RegisterTime,IsDeleted,DeleterUserId,LastModificationTime,LastModifierUserId,CreationTime,CreatorUserId ");
			strSql.Append(" FROM t_business ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from t_business T ");
