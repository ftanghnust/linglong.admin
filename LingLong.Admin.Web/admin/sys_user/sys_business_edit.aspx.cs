...
{
    public partial class sys_business_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == DTEnums.ActionEnum.Edit.ToString())
            {
                ...
                if (!new BLL.t_business().Exists(this.id))
                {
                    JscriptMsg("记录不存在或已被删除！", "back");
                    return;
                }
            }
            if (_action == DTEnums.ActionEnum.Delete.ToString()) {
                this.action = DTEnums.ActionEnum.Delete.ToString();//修改类型
                if (!int.TryParse(Request.QueryString["id"] as string, out this.id))
                {
                    JscriptMsg("传输参数不正确！", "back");
                    return;
        ...
        private void ShowInfo(int _id)
        {
            ...
            Model.t_business model = bll.GetModel(_id);
        ...
        private bool DoAdd()
        {
            Model.t_business model = new Model.t_business();
            BLL.t_business bll = new BLL.t_business();
        ...
        protected void DoDelete() {
            BLL.t_store_business bll = new BLL.t_store_business();
            bll.Delete(this.id);
            JscriptMsg("删除信息成功！", "sys_business_list.aspx");
        }
        //保存
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_business", DTEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("修改信息成功！", "sys_business_list.aspx");
            }
            else //添加
            {
                ChkAdminLevel("sys_business", DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加信息成功！", "sys_business_list.aspx");
            }
        }
    }
}